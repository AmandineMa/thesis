...
Once the entities are stabilised, geometric reasoners are applied to them to extract symbolic facts. In the current version of the system, the computed facts are \emph{isOnTopOf}, for an object on top of another with a direct contact, \emph{isInside}, for a block in a compartment, \emph{isVisibleBy}, assessing if an agent could see the object or not from his position, and \emph{isReachableBy}, assessing if an object can be taken by an agent. All these facts are sent to the robot's semantic knowledge base, where reasoners will deduce further facts. For example, if a block is in a compartment, thanks to inverse property \emph{hasInside} the fact that the compartment has the block inside is computed. In the same way, if this compartment is on top of the table, the block inside is computed to be above the table (\emph{isAbove)} thanks to chain axiom.
...
\begin{quote} 
	\centering 
	\emph{``Do you mean the block with a green triangle or the block with a blue triangle?''}
\end{quote}
