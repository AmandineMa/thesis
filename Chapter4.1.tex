...
\section{Description of deliberative architecture}\label{sec:globalarchi}\todo{est-ce qu'il faut charcuter des détails ?, acronyms}
...
The work presented about the supervision in this section is an early version of \acrfull{jahrvis} presented in Chapter~\ref{chapter:chap6}. It integrates on one hand the decision and control for the direction-giving task, and on the other hand the implementation in this task context of the metrics presented in Chapter~\ref{chapter:chap7}, to measure the Quality of Interaction.
...
As presented in the introduction (Section~\ref{chap8:sec:intro}), we performed a user study with the mall customers in January 2020, on one week, after the 14 months deployments. It was carried out in collaboration with Kathleen Belhassein, the leader of this study. She elaborated the study procedure and performed the analysis of the collected data. And, she participated in the organization and implementation of the study, the creation of the behavioral video coding scheme, the video coding. The author was in charge of the robotic system, and participated in the organization and implementation of the study, the creation of the behavioral video coding scheme, and the video coding. 

This study was an opportunity to investigate the impact of different communicative strategies used by the robot in the context of a real environment task. Indeed, Pepper is a robotic platform with a certain number of communication means, among which the tablet, gestures, movements, dialogue, or even lights. In this study, Pepper used different communicative channels to describe the route to take: 
\begin{bulletList}
	\item The dialogue, verbally explaining the route
	\item The tablet, which transcribed Pepper's oral speech
	\item The lights, to inform that it was listening or reasoning
	\item The gestures, to give rhythm to its speech or to point an element on the road 
	\item Its movements, to take the perspective of his interlocutor and adapt his route description
\end{bulletList}

This user study aims at comparing three different versions of the robotic system, in order to evaluate the impact of the different communicative strategies on the efficiency and quality of the interaction perceived by the customers: is the route description more efficient if the robot positions itself in space to point, taking the perspective of its interlocutor? Is the interaction more pleasant for the users? Is the quality of the interaction perceived as better?
More precisely, we have chosen to compare three versions of the system:
\begin{bulletList}
	\item Condition 1 : Dialogue only
	\item Condition 2 : Dialogue + Pointing
	\item Condition 3 : Dialogue + Pointing + Navigation (the complete system)
\end{bulletList}

In Condition 1, the robot performs the task only through dialogue. It verbally explains the route to be taken without ever pointing to an element or moving. Condition 2 integrates pointing gestures in parallel with the dialogue, and the possibility to rotate on itself to point to the desired element. Finally, Condition 3 is performed with the complete system as presented in Section~\ref{sec:globalarchi}, reasoning on the participant's perspective to move so that the participant can see the pointed element.
...
\begin{bulletList}
	\item Introduction: \textit{``Hi, my name is Pepper, I can help you find your way...''}
	\item Target: Verbalization of the direction of the desired store
	\item Path: Verbalization of the path to take to get to the desired store
	\item Comprehension question: \textit{``Did you understand?'', ``Have you seen the intersection?''}
	\item Repetition: \textit{``Should I show you again?''}
	\item Repositioning face: \textit{``Please, come in front of me''}
	\item Repositioning distance: \textit{``Can you come closer?''}
	\item Lateral repositioning: \textit{``Can you make a few steps on your left/right, please?''}
	\item Announcement of a move: \textit{``I'm going to move so I can show you.''}
	\item Announcement of path explanation: \textit{``Now I'm going to explain to you the route.''}
	\item OK
	\item End of interaction: \textit{I'm happy that I was able to help you. Okay, let's talk about something else...''}
...
Some items, such as repositioning requests or move announcements, were not present in the no travel conditions.

\subsection{Participants}

Two experimenters were present and visible to the participants in order to manage the registration systems and the visitors of the mall, to ensure the smooth running of each participation, and to administer the questionnaires as well as the effectiveness test. A third experimenter, being hidden from the participants by a panel, managed the launch of the Supervision. Indeed, some parameters had to be manually changed to switch between conditions, and it also ensured a clean start for each interaction. This launch was the only moment in a condition during which the experimenter intervened on the robotic system, otherwise the robot was acting completely autonomously. A fourth experimenter, from VTT, was present to provide translation and conduct the experiment in Finnish.
...
9 participants were excluded from the analyses due to technical problems during the experiment, which could later interfere with their evaluations of the different systems. Thus, 25 participants were included in the analysis, 17 women and 8 men with their ages still ranging from 20 to 68 ($M=37.12$, $SD=10.11$).

\subsection{Tools for Data Collection}

In order to collect the participants' opinions about each condition, we prepared two questionnaires for them to fill: \acrshort{perdita}, a questionnaire dedicated for \acrshort{hri} user studies and a questionnaire that we devised for this particular user study. These questionnaires were presented in Finnish to the participants (both of them were translated double-blind) but will be presented in English in this section. At the end of each condition, each participant had to fill both.
...
The appropriateness of the robotic system decisions was evaluated with the questionnaire \acrfull{perdita}~\cite{devin_2018_evaluating} which in the form of a scale from 1 to 7. It was developed at LAAS-CNRS and has been proposed to the robotic community as a basis for the development of a tool to evaluate the decisional aspects of a robotic agent in a joint action. It is not specific to a robotic platform or to a particular task. The version used in this study has several dimensions, each measuring a specific aspect of joint action (Table~\ref{chap8:table:perdita}, Appendix~\ref{app:perdita} for the English version of the questionnaire and Appendix~\ref{app:perdita_fi} for the Finnish version of the questionnaire which was given to the participants). It was used in two previous studies~\cite{devin_2018_evaluating,buisan_thesis}. After the first one~\cite{devin_2018_evaluating}, the questionnaire and the dimensions were refined as the reliability was too low. Then, in the second study~\cite{buisan_thesis} with the updated questionnaire, the reliability was good enough which allowed us to keep this version of the questionnaire for our experimentation. 
...
An additional questionnaire was used to collect the participants' evaluation of the overall quality of the interaction (see Appendix~\ref{app:add_questionnaire} for the English version and Appendix~\ref{app:add_questionnaire_fi} for the Finnish version given to the participants). Originally, we devised this questionnaire in order to comparer its result with the \acrshort{qoi} measured by the robot (see Chapter~\ref{chapter:chap7}). However, the integration of the \acrshort{qoi} Evaluator was not mature enough at the time of the user study and so there were no robotic results to compare the questionnaire to. But, interestingly, as we will see in Section~\ref{chap8:subsec:us_results}, this questionnaire gave valuable results.
Participants were asked to rate their interaction with Pepper on a scale of -1 (poor) to +1 (good). Two additional open-ended questions were asked: ``Have you seen everything Pepper has indicated to you?'' and ``Did you understand the path you have to take?''. 
...
The videos were annotated using the software ELAN\footnote{\url{https://archive.mpi.nl/tla/elan}}~\cite{wittenburg_2006_elan}, in order to analyze communicative elements between the participant and the robot (such as gazes or positions) in terms of frequency of occurrence, durations, and sequences of occurrence. A behavioral video coding scheme was devised prior to the user study, based on the tests conducted in the laboratory, allowing the analysis of these elements. This coding scheme can be found in Appendix~\ref{app:coding_r} for the observed robot behaviors and in Appendix~\ref{app:coding_h} for the observed human behaviors. 

10\% of the videos were analyzed in double-blind. The Cohen's kappa coefficients were all higher than 0.8, ensuring good inter-rater reliability.
...
The study was a within-subjects experiment, whose goal was to evaluate the impact of three different versions of the system, each reflecting a particular communicative strategy, on the effectiveness of the route description and the interaction quality. 

The task was for participants to ask the robot where a particular store was located, so that the robot would explain the route to get there.

The independent variable was the version of the system used. The first version, corresponding to Condition 1, was performed using only dialog (verbal and visual\footnote{Pepper verbally explained the route, and the content of its speech was simultaneously displayed on the display on its chest}). We had disabled the subtask ``Ensuring Correct Placement'' and the pointing actions (see Figure~\ref{fig:chap3_SM}). The second version of the system, corresponding to the Condition 2, added the pointing of the passage and the target (eventually rotating to be able to do so). However, the ``Ensuring Correct Placement'' subtask remained disabled. Finally, the last version, corresponding to Condition 3, was performed with the complete system, as described in Section~\ref{sec:globalarchi}, \ie re-enabling the ``Ensuring Correct Placement'' subtask. 

Each participant was confronted with the three conditions, the order being counterbalanced for each.

Three stores were chosen as targets to ask about, based on their visibility and distance from where the robot was deployed: 
\begin{bulletList}
...
Stores were also counterbalanced within the experimental conditions.

The participant was first greeted by the experimenters in the interaction area at the time of the appointment. It would sign the consent form and image rights release. One of the experimenters would then explain to the participant how the interaction would take place, without mentioning the purpose of the study. The participant was then asked to come in front of the robot to start a habituation phase, in which the robot demonstrated its capabilities and the behaviors it might exhibit during the study. During this phase, the robot would give a route direction, pointing and moving around the participant. 
...
In this section, we will present the results obtained from the data analysis for the two questionnaires and the video observation.

\subsubsection{\acrshort{perdita}}

Data from \acrshort{perdita} were analyzed using the Friedman test because the measures were quantitative and non-parametric. Indeed, nothing proves that each gap between two numbers of the scale are identical (\eg a person could easily change their answer from 2 to 3 but a lot less from 6 to 7). It is separately performed on each dimension of the questionnaire. To use such a test allows to know if the answers filled by participants for a given dimension and a given condition are significantly different from the ones given for the same dimension in the other conditions. Analyses revealed that only the dimension Collaboration showed a significant trend, $F(2,25) = 5.96, p = 0.0509$, with the effect size measured by Kendall's W being small ($W=0.1192$). Since it was significant, this was followed by a pairwise comparison of the conditions using the Wilcoxon signed-rank test. The p-values were adjusted\footnote{P-values are adjusted because several tests were performed on the same data.} using the Bonferroni-Holm method. The results do not show a significant comparison (see Table~\ref{chap8:table:wilcox_perdita}). 

Analyses showed no significant effect of the different experimental conditions on the dimensions Perceived robot skill ($F(2,25)=5.45, p=0.0655$), Interaction ($F(2,25)=4.89, p=0.0867$), Verbal ($F(2,25)=2.85, p=0.240$) and Action ($F(2,25)=4.98, p=0.0830$).

\begin{table}[]
	\centering
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lccccccccc}
			Dimension     &  y &  group1 & group2 &  n1 &  n2 & statistic & p &  p.adj &p.adj.signif \\ \hline\hline
			Action  & data  & 1 & 2  & 25  & 25    &236 & 0.136 & 1 & ns       \\\hline
			Action& data& 1  & 3  & 25  & 25  & 268   & 0.397 & 1 & ns       \\\hline
			Action& data& 2  & 3  & 25  & 25  & 372   & 0.246 & 1 & ns       \\\hline
			Collaboration & data& 1  & 2  & 25  & 25  & 236   & 0.139 & 1 & ns       \\\hline
			Collaboration & data& 1  & 3  & 25  & 25  & 235   & 0.134 & 1 & ns       \\ \hline
			Collaboration & data& 2  & 3  & 25  & 25  & 326   & 0.793 & 1 & ns       \\ \hline
			Competence    & data& 1  & 2  & 25  & 25  & 226   & 0.092 & 1 & ns       \\ \hline
			Competence    & data& 1  & 3  & 25  & 25  & 216   & 0.062 & 0.868  & ns       \\ \hline
			Competence    & data& 2  & 3  & 25  & 25  & 314   & 0.977 & 1 & ns       \\ \hline
			Interaction   & data& 1  & 2  & 25  & 25  & 254   & 0.259 & 1 & ns       \\ \hline
			Interaction   & data& 1  & 3  & 25  & 25  & 238   & 0.15  & 1 & ns       \\ \hline
			Interaction   & data& 2  & 3  & 25  & 25  & 296   & 0.748 & 1 & ns       \\ \hline
			Verbal& data& 1  & 2  & 25  & 25  & 202   & 0.032 & 0.48   & ns       \\ \hline
			Verbal& data& 1  & 3  & 25  & 25  & 268   & 0.396 & 1 & ns       \\ \hline
			Verbal& data& 2  & 3  & 25  & 25  & 396   & 0.102 & 1 & ns       \\ \hline
		\end{tabular}%
	}
	\caption{Results of the Wilcoxon signed-rank test on the answers given to the \acrshort{perdita} questionnaire. Comparisons are made between all conditions for each dimension of the questionnaire. No comparison is significant.}
	\label{chap8:table:wilcox_perdita}
\end{table}
...
\begin{table}[]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{ccccccccc}
			\hline
			 y     & group1 &  group2 &  n1 &  n2 &  statistic & p & p.adj & p.adj.signif \\ \hline
		Interaction note     & 1     & 2     & 25& 25& 16.5     & 0.011    & \textbf{0.023}       & *   \\ \hline
			Interaction note     & 1     & 3     & 25& 25& 5.0      & 0.002    & \textbf{0.007}       & **  \\ \hline
		Interaction note & 2     & 3     & 25& 25& 42.0     & 0.506    & 0.506& ns  \\ \hline
		\end{tabular}%
	}
...
Responses to the two open-ended questions in the supplemental questionnaire (``Did you see all the items Pepper showed you?'' (question 1) and ``Did you understand the path you need to take?'' (question 2) were classified into two categories: ``Yes'' and ``No''. The responses indicating that they only partially understood the path, or that they only saw some of the elements shown by Pepper were counted as a negative response. These data were analyzed with Cochran's Q test as there are non-parametric qualitative data with three conditions to compare. Affirmative responses appear to be more frequent in experimental Conditions 2 and 3 (see Figure~\ref{chap8:fig:yn_QA} and Table~\ref{chap8:table:contingency_table_QA}), which is confirmed by the analyses related to question 1 which show that the answer given depends on the experimental condition ($Q(2,25)=9.1, p<0.05$). 
...
\begin{table}[]
	...
	\resizebox{0.6\textwidth}{!}{%
		\begin{tabular}{l|lllll}
			\hline
			\multicolumn{1}{c|}{}            &     & Condition 1          & Condition 2          & Condition 3          & Row Total \\ \cline{2-6}
			\multicolumn{1}{c|}{}            & Yes & \begin{tabular}[c]{@{}l@{}}10\\ 16.333\\ 0.400\end{tabular} & \begin{tabular}[c]{@{}l@{}}20\\ 16.333\\ 0.800\end{tabular} & \begin{tabular}[c]{@{}l@{}}19\\ 16.333\\ 0.760\end{tabular} & 49        \\ \cline{2-6}
			\multicolumn{1}{c|}{}            & No & \begin{tabular}[c]{@{}l@{}}15\\ 8.667\\ 0.600\end{tabular}  & \begin{tabular}[c]{@{}l@{}}5\\ 8.667\\ 0.200\end{tabular}   & \begin{tabular}[c]{@{}l@{}}6\\ 8.667\\ 0.240\end{tabular}   & 26        \\ \cline{2-6}
			\multicolumn{1}{c|}{\multirow{-4}{*}{\rotatebox[origin=c]{90}{\large Question 1}}} &     & \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & 75        \\ \hline\hline
			 &     & Condition 1          & Condition 2          & Condition 3          & Row Total \\ \cline{2-6}
			 & Yes & \begin{tabular}[c]{@{}l@{}}12\\ 16.333\\ 0.480\end{tabular} & \begin{tabular}[c]{@{}l@{}}20\\ 16.333\\ 0.800\end{tabular} & \begin{tabular}[c]{@{}l@{}}17\\ 16.333\\ 0.680\end{tabular} & 49        \\ \cline{2-6}
			 & No & \begin{tabular}[c]{@{}l@{}}13\\ 8.667\\ 0.520\end{tabular}  & \begin{tabular}[c]{@{}l@{}}5\\ 8.667\\ 0.200\end{tabular}   & \begin{tabular}[c]{@{}l@{}}8\\ 8.667\\ 0.320\end{tabular}   & 26        \\ \cline{2-6}
			\multirow{-4}{*}{\rotatebox[origin=c]{90}{\large Question 2}}      &     & \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & 75        \\ \hline
		\end{tabular}%
	}
	\caption{Contingency table of the answers given to the two open-ended questions of the additional questionnaire according to the experimental conditions.}
	\label{chap8:table:contingency_table_QA}
\end{table}
...
We wanted to investigate the effect of the requested store on the type of answers, independently of the condition. For question 1, the type of the given answer does not depend on the store requested, ($Q(2,25)=4.9, p=0.0863$). On the other hand, it does depend on the requested store for question 2, ($Q(2,25)=11.4, p<0.01$). According to McNemar's test, the difference is significant between the stores City Kulta and Pentik ($p<0.05$) but not between the other stores (see Figure~\ref{chap8:fig:stores_comp_QA}). Participants seem to have more difficulty understanding the path to City Kulta (store far away and not visible) compared to the Pentik store.

\subsubsection{Video Observation}
Similarly to the analysis for the additional questionnaire, the Cochran's Q test was used to compare between the conditions, the answers (``Yes'' or ``No'') given to the robot during the task when it asked them if they had understood. The analysis rejected the independence between the given answer and the condition ($p<0.01$) (see Table~\ref{chap8:tab:contingency_comp_video}). According to post-hoc comparisons using the McNemar's test, the difference is significant between condition 1 and 2 ($p<0.05$), but not between condition 1 and 3 and between condition 2 and 3. Thus, participants appear to understand Pepper's directions better in Condition 2 than in Condition 1.
...
\begin{table}[]
	...
	\resizebox{0.6\textwidth}{!}{%
		\begin{tabular}{lllll}
			\hline
			& Condition 1 & Condition 2 & Condition 3  & Row Total \\ \hline
			Yes & \begin{tabular}[c]{@{}l@{}}17\\ 21.000\\ 0.680\end{tabular} & \begin{tabular}[c]{@{}l@{}}24\\ 21.000\\ 0.960\end{tabular} & \begin{tabular}[c]{@{}l@{}}22\\ 21.000\\ 0.880\end{tabular} & 63        \\ \hline
			No & \begin{tabular}[c]{@{}l@{}}8\\ 4.000\\ 0.320\end{tabular}   & \begin{tabular}[c]{@{}l@{}}1\\ 4.000\\ 0.040\end{tabular}   & \begin{tabular}[c]{@{}l@{}}3\\ 4.000\\ 0.120\end{tabular}   & 12        \\ \hline
			& \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & 75        \\ \hline
		\end{tabular}%
	}
	\caption{Contingency table of the answers given to the yes/no questions asked by Pepper according to the experimental conditions.}
	\label{chap8:tab:contingency_comp_video}
\end{table}
...
The data do not follow a normal distribution, which requires the use of non-parametric tests. Friedman's test reveals an effect of the condition on the percentage of time spent looking in the direction of the path ($F=20.33, p<.01$). Wilcoxon tests show a significant difference between condition 1 and 2 ($p<.01$) and between condition 1 and 3 ($p<.01$) (see Figure~\ref{chap8:fig:diagram_video}). There was no significant difference between condition 2 and 3.

Finally, other analysis are presented in Table~\ref{chap8:tab:proba_video}.
...
19 participants perceived the difference between all conditions (see Figure~\ref{chap8:fig:interview_diff}), and 7 perceived only the difference between condition 1 and the other two conditions. 12 participants preferred Condition 2, 8 participants preferred Condition 3, and 4 preferred Condition 1 (see Figure~\ref{chap8:fig:interview_pref}). Condition 1 seemed to be the least preferred (13 participants, compared to 3 for condition 2 and 4 for condition 3) (see Figure~\ref{chap8:fig:interview_least}).
...
\begin{figure}[!htb]
	...
	\subfloat[]{\label{chap8:fig:interview_diff}\includegraphics[width=0.49\linewidth]{figures/chapter2/Difference_conditions.pdf}}\hfill
	
	\subfloat[]{\label{chap8:fig:interview_pref}\includegraphics[width=0.49\linewidth]{figures/chapter2/Preferred_condition.pdf}}\hfill
	\subfloat[]{\label{chap8:fig:interview_least}\includegraphics[width=0.49\linewidth]{figures/chapter2/Least_preferred_condition.pdf}}
