...
\begin{table}[]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{c|c|c}
			...
			\begin{tabular}[c]{@{}c@{}}Limiting/Adaptive\\ Useless/Useful\\ Unsatisfying/Satisfying\\ Disturbing/Acceptable\\ Ineffective/Effective\end{tabular} \\ \hline
			Interaction &
			\makecell{In your opinion, \\the interaction with the robot was:} &
			\begin{tabular}[c]{@{}c@{}}Negative/Positive\\ Unpractical/Practical\\ Complicated/Simple\\ Ambiguous/Clear\\ Unpleasant/Pleasant\end{tabular} \\\hline
			Perceived robot skill &
			\makecell{In your opinion, \\the robot is rather:} &
			\begin{tabular}[c]{@{}c@{}}Not reactive/Reactive\\ Inefficient/Efficient\\ Unintelligent/Intelligent\\ Unskilled/Skilled\end{tabular} \\\hline
			Verbal &
			...
			\begin{tabular}[c]{@{}c@{}}Unsuitable/Suitable\\ Uncomfortable/Accomodating\\ Unpredictable/Predictable\\ Not reassuring/Reassuring\end{tabular}
		\end{tabular}%
	}
	\caption{English translation of the questions for each dimension of \acrshort{perdita}. The used questionnaire, in Swedish, is in Appendix~\ref{app:perdita}.}
	\label{chap8:table:perdita}
\end{table}

\subsubsection{Additional Questionnaire}
An additional questionnaire was used to collect the participants' evaluation of the overall quality of the interaction. Originally, we devised this questionnaire in order to comparer its result with the \acrshort{qoi} measured by the robot (see Chapter~\ref{chapter:chap7}). However, the integration of the \acrshort{qoi} Evaluator was not mature enough at the time of the user study and so there were no robotic results to compare the questionnaire to. But, interestingly, as we will see in Section~\ref{chap8:subsec:us_results}, this questionnaire gave valuable results.
...
\subsubsection{Additional Questionnaire}
The interaction score given by the participants when asked to rate their interaction on a scale of -1 to +1 correlated with the Interaction dimension of the PeRDITA questionnaire ($r = 0.85, p<0.05$). The score were scaled from 0 to 2 (instead of -1 to +1) to ease the analysis. They were analyzed with the Friedman test, since the data do not follow a normal distribution (the Shapiro-Wilk test was significant ($W=0.829, p=7.153e-08$)).

Analysis reveals a significant effect of the conditions on the interaction score of the additional questionnaire ($F(2,25) = 10.7, p<0.01$) with a small effect size ($W=0.215$). Pairwise comparison performed with the Wilcoxon test and adjusted p-values, shows a significant difference between Condition 1 ($M=1.18, SD=0.69$) and 2 $(M=1.5, SD=0.6; p<0.05$), and between Condition 1 ($M=1.18, SD=0.69$) and 3 ($M=1.56, SD=0.46; p<0.01$) (see Table~\ref{chap8:table:wilcox_QA}). No significant difference was found between Condition 2 and Condition 3, $p=0.506$ (see Figure~\ref{chap8:fig:graph_QA}).
...
\begin{table}[]
	...
	\caption{Results of Wilcoxon signed-ranks test on responses about interaction of the additional questionnaire. Comparisons between group 1 and 2 ($p<0.05$) and between group 1 and 3 ($p<0.01$) were significant.}
	\label{chap8:table:wilcox_QA}
\end{table}

Responses to the two open-ended questions in the supplemental questionnaire (``Did you see all the items Pepper showed you?'' and ``Did you understand the path you need to take?'') were classified into two categories: ``Yes'' and ``No''. The responses indicating that they only partially understood the path, or that they only saw some of the elements shown by Pepper were counted as a negative response. These data were analyzed with Cochran's Q test. Affirmative responses appear to be more frequent in experimental conditions 2 and 3 (see Figure~\ref{chap8:fig:yn_QA} and Table~\ref{chap8:table:contingency_table_QA}), which is confirmed by the analyses for question 1 which show that the answer given depends on the experimental condition ($Q(2,25)=9.1, p<0.05$). Post-hoc comparisons were performed via the McNemar test.
...
\begin{table}[]
	...
	\resizebox{0.6\textwidth}{!}{%
		\begin{tabular}{l|lllll}
			\hline
			\multicolumn{1}{c|}{}            &     & Condition 1          & Condition 2          & Condition 3          & Total \\ \cline{2-6}
			\multicolumn{1}{c|}{}            & Oui & \begin{tabular}[c]{@{}l@{}}10\\ 16.333\\ 0.400\end{tabular} & \begin{tabular}[c]{@{}l@{}}20\\ 16.333\\ 0.800\end{tabular} & \begin{tabular}[c]{@{}l@{}}19\\ 16.333\\ 0.760\end{tabular} & 49        \\ \cline{2-6}
			\multicolumn{1}{c|}{}            & Non & \begin{tabular}[c]{@{}l@{}}15\\ 8.667\\ 0.600\end{tabular}  & \begin{tabular}[c]{@{}l@{}}5\\ 8.667\\ 0.200\end{tabular}   & \begin{tabular}[c]{@{}l@{}}6\\ 8.667\\ 0.240\end{tabular}   & 26        \\ \cline{2-6}
			\multicolumn{1}{c|}{\multirow{-4}{*}{\rotatebox[origin=c]{90}{\large Question 1}}} &     & \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & \begin{tabular}[c]{@{}l@{}}25\\ 0.333\end{tabular}          & 75        \\ \hline
			 &     & Condition 1          & Condition 2          & Condition 3          & Total \\ \cline{2-6}
			 & Oui & \begin{tabular}[c]{@{}l@{}}12\\ 16.333\\ 0.480\end{tabular} & \begin{tabular}[c]{@{}l@{}}20\\ 16.333\\ 0.800\end{tabular} & \begin{tabular}[c]{@{}l@{}}17\\ 16.333\\ 0.680\end{tabular} & 49        \\ \cline{2-6}
			 & Non & \begin{tabular}[c]{@{}l@{}}13\\ 8.667\\ 0.520\end{tabular}  & \begin{tabular}[c]{@{}l@{}}5\\ 8.667\\ 0.200\end{tabular}   & \begin{tabular}[c]{@{}l@{}}8\\ 8.667\\ 0.320\end{tabular}   & 26        \\ \cline{2-6}
