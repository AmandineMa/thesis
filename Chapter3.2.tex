...
%	\centering
	...
	\subfloat[One initial set-up (top view)]{\label{chap6:subfig:task_setup}\includegraphics[width=0.5\linewidth]{figures/chapter2/task_setup.pdf}}
	\hspace*{\fill}
...
\thispagestyle{example}
\begin{figure}[!hbtp]
	\subfloat[The human is picking the blue cube while the robot is not looking as it is picking the green cube. Consequently, the robot does not know that the human performed an action. ]{\label{chap6:fig:action_monitoring_photos_not_looking}\includegraphics[width=0.45\linewidth]{figures/chapter2/looking_somewhere_else.jpg}}\hfill
	\subfloat[The robot is looking again in the human direction and observes that the blue cube is not on the table anymore. It deduces that the human has taken it and so performed the action in the plan.]{\label{chap6:fig:action_monitoring_photos_now_looking}\includegraphics[width=0.45\linewidth]{figures/chapter2/now_looking.jpg}}\hfill
...
Having the robot able to enunciate or ask information to the human is important, but that it is able to understand communication from them is as well. The human should be allow to communicate about the plan such as to ask precision about a given action, to signal that something is not going well or to ask the robot to perform an action. We focused on the latter in collaboration with Guillaume Sarthou. 
