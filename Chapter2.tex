...
\subsubsection{Shared Plan Representation}\label{chap2:subsubsec:shared_p}
...
The second type of shared plans is an extension of the work of Devin about postponing some decisions from planning time to execution time about the actor of some actions and some parameters~\cite{devin_2017_decisions}. In works previous to Devin, all the actions of the computed plans were
...
\begin{algorithm}
	...
	\begin{algorithmic}
		\Function{UpdatePlan}{}
		\ForEach{primTask($i$, \algNormTextSub{State}{i},$preds_i$,$decompoOf_i$),\\\hfill State $=$ \algConst{PLANNED}$,i \in \mathbb{N}$}{planPrimTasks[]}
		\State $preds$ $\coloneqq$\textsc{FindAll}($j$) 
		\\\hfill such as primTask($j$,\algNormTextSub{State}{j},$preds_j$,$decompoOf_i$) $\in$ planPrimTasks[],\\ \hfill $j \in preds_i$, \algNormTextSub{State}{j} $\neq$ \algConst{EXECUTED}
		\If{$preds = \emptyset$}
		\State primTask($i$,\algConst{PLANNED},$preds_i$) 
...
\begin{algorithm}
	\caption{Update of abstract tasks on action update}
	\label{chap2:algo:UATO}
	\begin{algorithmic}
		\Function{UpdateAbstractTasksToOngoing}{$i$}
		\If{$\exists$ abstractTask($i$,\algNormTextSub{State}{i},$decompoOf_i$) and State $=$ \algConst{PLANNED}}
		\State abstractTask($i$,\algConst{PLANNED},$decompoOf_i$) $\coloneqq$ abstractTask($i$,\algConst{ONGOING},$decompoOf_i$)
		\State \textsc{UpdateAbstractTasksToOngoing($decompoOf_i$)}
		\EndIf
		\EndFunction
	\end{algorithmic}
