...
Thus, the Situation Assessment outputs data, which we call \textit{symbolic facts}, such as \mbox{\textit{isOnTopOf}(cube\_1,cube\_3)} or \mbox{\textit{isReachableBy}(cube\_2,human\_0)}.
...
The Supervisor can subscribe to a given type of fact, allowing to receive every fact update of this type (\eg the Supervisor can ask to receive every update (addition or deletion) of any fact belonging to the type \textit{isOnTopOf}(Cube,Table)). It can also query a information when needed (\eg the Supervisor can ask the human understandable name of pick\_action which is pick).
...
\subsubsection{The Choice of the Framework}
...
\subsubsection{Programming with Jason}
...
\paragraph{Actions} To give an idea of what looks like the Java program of an action, here is an example of a Java function for the action |.print| in Listing~\ref{chap2:lst:print}.
...
\subsubsection{Jason Integration with ROS}
...
\todo{to formulate}
\begin{itemize}
	\item generic supervisor, not specific to a task, can handle a lot of various plans
	\item leave some decisions to the human: handle conditional plan and handle agent X~\cite{devin_2017_decisional} (two different ways)
	\item take human into account: abilities, knowledge about the task progress, belief divergences
	\item to do relevant communication actions (verbal, non-verbal)
	\item considerate not only tasks but also interaction sessions
	\item recognize the human actions
	\item to know if the interaction is going well to be able to react on it
	\item active perception (object/human search)
	\item handle contingencies
\end{itemize}

For each processes, we implemented a ROS-Jason Agent, with the wanted behavior coded in ASL and the needed customizations added in Java.


\paragraph{Interaction Session}
...
\subsection{Knowledge Representations and Management}
action model defined in ontologenius (type, verbalizations);
mementar timeline; 
belief divergence;
what the human knows about the current plan

\subsection{Human Actions Monitoring}
developed model:
internal action model with preconditions, ongoing effects and final effects;
state machine
...
integrate multiple humans

improve goal management and interaction
...
The section is organized as follows. First, we briefly discuss related work and the main challenges. In section~\ref{sec:levels} we present the representation of human-robot collaborative activity which we use and its hierarchical  decomposition. Finally, in sections \ref{sec:eval} and \ref{sec:metrics}, we introduce our concept and proposed set of metrics to evaluate the Quality of Interaction.
