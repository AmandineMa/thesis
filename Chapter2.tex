...
The Supervisor can subscribe to a given type of fact, allowing to receive every fact update of this type (\eg the Supervisor can ask to receive every update (addition or deletion) of any fact belonging to the type \fact{isOnTopOf}{Cube}{Table}) (\eg it will receive the addition of \fact{isOnTopOf}{cube\_2}{table\_1} but not \fact{isOnTopOf}{spoon}{table\_1}). It can also query a information when needed (\eg the Supervisor can ask the human understandable name of pick\_action which is ``pick'').
...
\section{Representation of a Human-Robot collaborative activity}\label{chap2:sec:levels}\todo{modifs à faire}
...
%		Subtasks are not considered as a ``real'' level of the interaction session, specially to evaluate the QoI, as it may exist or not according to the task.
		}
	\label{fig:levels}
\end{figure}
...
The \acrfull{hpm} keeps track of the estimated human mental state about the ongoing shared plan. The role of this process is central, as it receives the data about the human action monitoring, it deduces what the human might or might not know about the plan or action executed by the robot, and requests the communication to perform to the \acrfull{cm}. This component is closely link to Ontologenius 

When a shared goal starts, it receives from the \acrshort{rpm} the list of primitive tasks composing the plan. The action states are updated with the same algorithm as the \acrshort{rpm}, the Algorithm~\ref{chap2:algo:UP} (with the function updating the abstract tasks being not used). We distinguish two cases of |todo| actions, the actions to be performed by the human and the ones to be performed by the robot.

We present the Algorithm~\ref{chap2:algo:h_todo}, describing what happens in the~\acrshort{hpm} when it computes that the human has an action to perform. It only shows when everything goes well, \ie that the human perform the right action in the allocated time. The way the robot reacts if the human is not acting, by communicating, is described with Algorithm~\ref{chap2:algo:h_todo_conting}.
...
\begin{algorithm}[!htb]
	...
	\While{$t_{current} < T_{timeout}$ and not $matchingAction$}
		\If{\textsc{Match}(primTask,$monitoredAction_{\text{STARTED,PROG}}) = true$}
	...
	$matchingAction \coloneqq false$
	\While{$t_{current} < T_{timeout}$ and not $matchingAction$}
		\If{\textsc{Match}(primTask,$monitoredAction_{\text{ACHIEVED}}) = true$}
		\State $matchingAction \coloneqq true$
		\EndIf
	\EndWhile
	\If{$t_{current} > T_{timeout}$} \Return \EndIf
	\State primTask($i$,\algConst{ONGOING},$name$,human,$params$,$preds$) 
...
\begin{algorithm}[!htb]
	\caption{Handling of action todo not performed by human in \acrshort{hpm}}
	\label{chap2:algo:h_todo_conting}
	\begin{algorithmic}
		\Function{hey}{}
		\EndFunction
	\end{algorithmic}
...
head manager: when robot execute an action, look the action object; when the robot speaks, look at the human. Programmed through channels.\todo{deplacer presentation dans mummer ici ou trop task specific?}
